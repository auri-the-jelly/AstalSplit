...
def is_64_bit(pid):
    ...
    for (s,e,perms,path) in iter_maps(pid):
        if "r-xp" in perms and path and path.endswith((".exe","(deleted)")) or path.endswith(".so") or path.startswith("/"):
            try:
...
if __name__ == "__main__":
    ...
    module_name = "DELTARUNE.exe"      # e.g. "game.exe" for Wine targets; leave empty if absolute
    base_off    = 0x8B2818
    offsets     = [0x0]
    ...
    try:
        ...
        if "r" not in perms:
            raise RuntimeError(f"Address 0x{final_addr:x} not readable (perms '{perms}')")
        ...
        if e.errno == errno.EIO:
            print("EIO while reading mem: usually unmapped address or not properly attached.")
